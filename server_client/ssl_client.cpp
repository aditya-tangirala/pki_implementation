...
int login(SSL *ssl)
{
	...
	printf("%s\n",result);
	if(strcmp("Please enter a password",result) == 0)
	{
		while(1)
		{
			bzero(result,sizeof(result));
			...
			SSL_read(ssl,result,50);
			printf("%s\n",result);
			if(strcmp("Login successful",result) == 0)
				break;
 		}
		return 1;
	}
...
int user_register(SSL *ssl)
{
	...
	bzero(result,sizeof(result));
		...
		SSL_read(ssl,result,50);
		printf("%s\n",result);
		if(strcmp("Successful registration",result) == 0)
                	return 1;
        }
        return 0;	
}
void beforechat(SSL *ssl)
{
	...
	while(1)
	{
		printf("Select an action to be performed:\n");
        	printf("a.Login\tb. Registration\n");
		scanf("\n%c",&choice);
		//printf("%c",choice);
		if(choice == 'a')
		{
