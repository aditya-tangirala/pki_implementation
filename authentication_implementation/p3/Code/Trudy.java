...
public class Trudy {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out
					.println("Trudy sends Kab{N4-1} as message 5 in 1st session");
			...
			int ticketLengthcbc = Integer
					.valueOf(inputline_cbc.substring(0, 2));
			int Kab_lengthcbc = Integer.valueOf(inputline_cbc.substring(2, 4));
			String ticketToBob_cbc = inputline_cbc.substring(4,
					ticketLengthcbc + 4);

			// Replaying message 3 between Alice and Bob to Bob.
			out1.println(inputline_cbc);
			System.out.println("Trudy replays message 3 to Bob");

			// Receiving message 4 from Bob - Kab{N2-1, N4}
			String lineFromBob_cbc = null;
			lineFromBob_cbc = in1.readLine();
			System.out.println("Trudy receives message 4 from Bob");
			...
			// Separating Kab{N4} from message 4 to use for reflection attack
			int lencbc = ((lineFromBob_cbc.length() - 8) / 2);
			String n3FromBob_cbc = lineFromBob_cbc.substring(8, lencbc + 8);

			System.out.println("Trudy opens a new session with Bob");
			// Re-sending ticket received from Alice and sent to Bob in the
			// first session along with Kab{N4}
			String toBob_cbc = String.valueOf(ticketLengthcbc)
					.concat(String.valueOf(Kab_lengthcbc))
					.concat(ticketToBob_cbc).concat(iv).concat(n3FromBob_cbc);
			out2.println(toBob_cbc);
			System.out
					.println("Trudy re-sends ticket from message 3 along with encryted nonce received from Bob in message 4");

			System.out.println("Trudy not authenticated as Alice !!!");
			// Receives Kab{N4-1, N5} from Bob
			String lineFromBob1_cbc = null;
			lineFromBob1_cbc = in2.readLine();
			System.out
					.println("Trudy receives message 4 from Bob in new session - contains Kab{N4-1}");
			...
			// Separating Kab{N4-1} from the previous message from Bob
			int len1cbc = (lineFromBob1_cbc.length() - 8) / 2;
			String sendToBobInFirstConnection_cbc = lineFromBob1_cbc.substring(
					8, len1cbc + 8);

			// Sending Kab{N4-1} as message 5 in 1st session with Bob
			String msg5ToBob_cbc = iv1.concat(sendToBobInFirstConnection_cbc);
			out1.println(msg5ToBob_cbc);
			System.out
					.println("Trudy sends Kab{N4-1} as message 5 in 1st session");
